...
setwd("C:/Users/Stef/Documents/Uni Ulm/WiWi/4. Semester/Bachelorarbeit/Problem Set")
ps.name = "InfluenceOfFemaleRoleModels"; sol.file = paste0(ps.name,"_sol.Rmd")
libs = c("haven", "dplyr", "ggplot2", "forcats", "gridExtra", "lfe", "texreg") # character vector of all packages you load in the problem set
...
This problem set is also published on `Github`: https://github.com/StefanieBuda/RTutorInfluenceOfFemaleRoleModels.
...
#>
#< fill_in
___ <- ___("FemaleRoleModels.dta")
#>
data_complete <- read_dta("FemaleRoleModels.dta")
```
...
A *dummy variable* is a special type of variable which can only take a value of 0 and 1. Therefore it is like a boolean indicating if something is true or false.
...
#< quiz "Structure of the data set"

question: But what is one observation?
sc:
    - a class 
    - a student *
    - a school
...
failure: Take a look at the prior exercise and try again.
...
#< quiz "Existence of NAs"

question: Does any of our variables contain NA's?
sc:
    - yes 
    - no *
    
success: Great, that's correct!
failure: Take a look at the summary again. If there are NA's present, it will appear as a new line starting with 'NA's'.
...
#< quiz "Percentage of women"

question: How many of the students in our data set are female?
sc:
    - 49.68 % 
    - 49.03 %
    - 44.88 % *
...
success: Great, that's correct!
failure: Take a look at the summary again. You have to look at the Mean for `female`.
...
#< hint
display("Think about what a value of 1 or 0 means for each variable. The first two rows are for the year 2015, the last two for the year 2016. The first and third row are the control groups, the second and fourth the treatment groups. Now assign it accordingly.")
#>
```
...
The pipe operator (%>%) is a neat way to shorten your code. It basically sends the information from one line directly to the next one. Like this you can concatenate several commands.  

We imported it within the package `dplyr`. For more information see https://CRAN.R-project.org/package=dplyr.
...
#< award "You got started!"

Great! You managed to solve the first exercise. You learned how to load the data, how to have a first look at it and even did some data manipulation.

You can look at all the awards you have collected so far by using the command `awards()` in a code box. 
...
#< quiz "Balance of freshman"

question: Is there a significant difference in the share of freshmen?
sc:
    - yes, there are significantly fewer freshmen in the treatment group  
    - no, there is no significant difference
    - yes, there are significantly more freshmen in the treatment group *
    
success: Great, that's correct!
failure: Take a look at the means and the p-value above. Then try again.
...
"The *John Henry effect* happens when comparison units work harder to compensate for not being offered a treatment." 

Source: Gertler et al., 2016, p.160.
...
"A *spillover* happens when an intervention affects a nonparticipant, and it might be positive or negative." 

Source: Gertler et al., 2016, p.163.
...
"The *Hawthorne effect* occurs when the mere fact that you are observing units makes them behave differently."
...
#< quiz "Treatment Effects"

question: Which of the treatment effects mentioned above is most likely to be a problem in our setting?
sc:
...
success: Great, that's correct!
failure: Maybe you should take a look at the info boxes. Try again.
...
#< quiz "Estimated difference is a lower bound"

question: Which of these choices fits the blank in the statement above?
sc:
    - more of an upper bound 
    - a quite exact estimation
    - more of a lower bound *
    
success: Great, that's correct!
failure: If the difference is underestimated, the actual impact should be higher then the estimation. Therefore the estimation is...
...
#< award "Pro field experimenter"

Great! You managed to solve the second exercise. You learned how to check the data for imbalances and learned something about particular effects that could affect your results, e.g. a spillover.
...
table_inter %>%
  ...
  theme_minimal()
#< hint
display("The first blank refers to the variable that should be shown on the x-axis. The second blank is a number: what factor do you have to multiply 0.18 by to get 18?")
#>
```
...
There are many more ways to create nice plots with `ggplot()` besides bar plots, for example
...
For more information on the package `ggplot2` see https://CRAN.R-project.org/package=ggplot2. 
...
Arranging your plots is especially useful when you want to compare the data shown in the plots, e.g. if the y-axis shows the same data. But if you do so, you have to be careful with the scaling of your axes: to do a proper comparison, both axes should have the same limits. Otherwise smaller differences might seem bigger than they actually are (or the other way round) just because of different scales.

For more information on the package `gridExtra` see https://CRAN.R-project.org/package=gridExtra. 
...
Great! You managed to solve the third exercise. You learned how to set up a plot to have a nice graphical representation of your data.
...
#< task
load("FemaleRoleModels_data")
load("FemaleRoleModels_table_inter")
#>
```
...
question: First of all - having our previous plots in mind - what are your expectations for the treatment effect?
sc:
    - positive effect, therefore a DID impact > 0 *
    - no effect at all, therefor a DID impact = 0
    - negative effect, therefore a DID impact < 0
    
success: Great, that's correct!
failure: Maybe you should take a look at the plot once more, then try again.
...
DID_impact
#>
DID_impact = (0.1846 - 0.1098) - (0.1073 - 0.1474)
DID_impact
```
...
#< quiz "Interpretation of the DID estimator"

question: The intervention increased the likelihood of enrolling in Intermediate Microeconomics by...
sc:
    - 11.5 percent.
...
#< award "Pro difference estimator"

Great! You managed to solve the fourth exercise. You learned the basics of the difference-in-differences method and calculated a first manual example for it.
...
#< quiz "Variable for dt_i"

question: Which of the variables in our data should be used for $dt_i$?
sc:
    - treat2016
    - year2016 *
    - took_intermediate
...
success: Great, that's correct!
failure: The variable we are looking for tells us something about the year. Try again.
...
#>
reg1 <- felm(took_intermediate ~ year2016:treat2015 + year2016 + treat2015|0|0|class_cluster, data = data)
```
...
#>
reg2 <- felm(took_intermediate ~ year2016:treat2015 + year2016 + treat2015 + female_prof + instate + freshman + american + ACumGPA + grade_principles + small_class|0|0|class_cluster, data = data)
```


#< info "Controls"

The controls we are adding are the following:

- `female_prof`: dummy variable for the gender of the professor teaching the class (1 for female, 0 for male)

- `instate`: dummy variable for the student's in-state status (1 for in-state, 0 for out-of-state)

- `freshman`: dummy variable for the student's year of study (1 for freshmen, 0 for higher year of study)

- `american`: dummy variable for nationality of the student (1 for American, 0 for international student)

- `ACumGPA`: the student's cumulative GPA (Grade Point Average)

- `grade_principles`: the student's grade in the Principles class

- `small_class`: dummy variable for the size of the class (1 for a small class, 0 for large class)
...
#< quiz "Significance of our estimator"

question: What level of significance do we get for our estimator?
sc:
    - 10 percent level
    - 1 percent level *
    - 5 percent level
...
success: Great, that's correct!
failure: Have a look at the starts shown in the table and try again.
...
#>
reg3 <- felm(took_another ~ year2016:treat2015 + year2016 + treat2015|0|0|class_cluster, data = data)

reg4 <- felm(took_another ~ year2016:treat2015 + year2016 + treat2015 + female_prof + instate + freshman + american + ACumGPA + grade_principles + small_class|0|0|class_cluster, data = data)
```
...
#>
reg5 <- felm(num_econclass ~ year2016:treat2015 + year2016 + treat2015|0|0|class_cluster, data = data)
...
#< award "Pro difference estimator in regressions"

Great! You managed to solve the fifth exercise. You learned how to use the difference-in-differences method for a linear regression how to interpret the results.
...
screenreg(list(reg9, reg10, ___, ___), 
          ...
          digits = 3)
#>
reg9 <- felm(Major_STEM ~ year2016:treat2015 + year2016 + treat2015 + female_prof + instate + freshman + american + ACumGPA + grade_principles + small_class|0|0|class_cluster, data = data)

reg10 <- felm(Major_Finance ~ year2016:treat2015 + year2016 + treat2015 + female_prof + instate + freshman + american + ACumGPA + grade_principles + small_class|0|0|class_cluster, data = data)

reg11 <- felm(Major_Business ~ year2016:treat2015 + year2016 + treat2015 + female_prof + instate + freshman + american + ACumGPA + grade_principles + small_class|0|0|class_cluster, data = data)

reg12 <- felm(Major_Marketing ~ year2016:treat2015 + year2016 + treat2015 + female_prof + instate + freshman + american + ACumGPA + grade_principles + small_class|0|0|class_cluster, data = data)


screenreg(list(reg9, reg10, reg11, reg12), 
          custom.model.names = c("Major STEM", "Major finance", "Major business", "Major marketing"), 
          omit.coef = "(female_prof)|(instate)|(freshman)|(american)|(ACumGPA)|(grade_principles)|(small_class)",
          reorder.coef = c(4, 2, 3, 1),
          digits = 3)
...
#< quiz "Effects on high-earning fields"

question: Were there any significant impacts on the high-earning field subjects?
sc:
    - yes, on majoring finance
    - yes, on majoring marketing 
    - yes, on majoring both finance and marketing
    - no, no significant impacts on either of them *
    
success: Great, that's correct!
failure: Take a look at the significance levels which are represented by stars. Try again.
...
#< quiz "Effects on low-earning fields"

question: Were there any significant impacts on the low-earning field subjects?
sc:
    - yes, on majoring arts
    - yes, on majoring humanities * 
    - yes, on majoring both arts and humanities
    - no, no significant impacts on either of them
...
```{r}
#< task
load("FemaleRoleModels_data_complete")
#>
```
...
#< quiz "Effects on male students"

question: Does any of the plots suggest an effect on male students?
sc:
    - yes, the plot for enrollment in Intermediate Micro
    - yes, the plot for majoring in economics
    - no, there is no evidence for an effect * 
    
success: Great, that's correct!
failure: Take a closer look a the plots and try again.
...
#< award "Pro problemset solver"

Great! You managed to solve the sixth and last exercise. You applied your knowledge from prior exercises to these new analyses.
...
You worked through this problem set about the influence of female role models on the choice of major. I hope you had fun while doing so and were able to learn something about programming in R and the statistical methods we used.
